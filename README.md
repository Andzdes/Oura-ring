...
```sh
git clone git@github.com:Andzdes/Oura-ring.git
cd Oura-ring
```

Create the server's `.env` using `.env.example` as the template. Set the existing
`TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `PUBLIC_APP_URL` and `BIND_PORT`.
...
The container binds only to localhost. Configure the existing reverse proxy for
